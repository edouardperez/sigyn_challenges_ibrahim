...
import { SingleArtifactToolGate } from "@/components/assistant-ui/single-artifact-tool-gate";
import { ToolFallback } from "@/components/assistant-ui/tool-fallback";
import { TooltipIconButton } from "@/components/assistant-ui/tooltip-icon-button";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

// Store toolkit in module scope to access in message rendering
let _toolkit: Toolkit | undefined;

export const Thread: FC<{ toolkit?: Toolkit }> = ({ toolkit }) => {
	_toolkit = toolkit;
	return (
	...
	return (
		...
		>
			<div className="aui-assistant-message-content wrap-break-word px-2 text-foreground leading-relaxed">
				<MessagePrimitive.Parts>
					{({ part }) => {
						...
						if (part.type === "tool-call") {
							// Try to render using toolkit if available
							const tool = _toolkit?.[part.toolName];
							if (tool && tool.type === "backend" && part.result !== undefined) {
								const ToolUI = tool.render({ result: part.result, toolCallId: part.toolCallId, addResult: undefined });
								if (ToolUI) {
									return (
										<SingleArtifactToolGate
				...
				{/* Show thinking indicator when message is being generated */}
				<AuiIf condition={(s) => s.message.role === "assistant" && s.message.content.length === 0 && s.thread.isRunning}>
